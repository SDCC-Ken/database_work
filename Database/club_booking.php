...
$SQL2 =  "SELECT Club_ID FROM club WHERE Club_ID = ".$_POST['CID'];
	...
	if ($bdate > $today && $stime < $etime && $rowcount1 > 0 && $rowcount2 > 0) {
		if ($rowcount3 > 0)
			...
			$SQL4 = "INSERT INTO  `booking` (`Room_ID` ,`Booking_Date` ,`Booking_STime`"." ,`Booking_ETime`".
				"  ,`Club_ID` ,`Booking_Purpose`) VALUE ('".
				$_POST['room']."','".$_POST['bdate']."','".$_POST['bstime']."','".
				$_POST['betime']."',".$_POST['CID'].",'".$_POST['BP']."')";
		...
		$message = $message . "Please check the value again.Wrong Date or Time or Room number or club number!";
};
if($_POST['Check_modify']=="GO"){
	...
	if ($bdate > $today && $stime < $etime && $rowcount1 > 0 && $rowcount2 > 0) {
		if ($rowcount3 > 0)
			...
			$SQL4 = "UPDATE booking SET ".
					"Room_ID = '".$_POST['room']."' , ".
					"Booking_Date = '".$_POST['bdate']."' , ".
					"Booking_STime = '".$_POST['bstime']."' , ".
					"Booking_ETime = '".$_POST['betime']."' , ".
					"Club_ID = '".$_POST['CID']."' , ".
					"Booking_Purpose = '".$_POST['BP']."' ".
					"WHERE Booking_ID = ".$_POST['booking_id']; 
			if(mysqli_query($con,$SQL4)){
				$message = $message . "Your booinking has been changeed.";
				$SQL5 = "SELECT * FROM booking WHERE Booking_Date = '".$_POST['bdate']."'
						 AND Booking_STime = '".$_POST['bstime']."'
						 AND Booking_ETime = '".$_POST['betime']."'
						 AND Room_ID = '".$_POST['room']."'";
...
?>
<p>It shows all the room booked by your club.</p>
<?php
$SQL = "SELECT * FROM booking WHERE Club_ID = ".$_SESSION['CID']." ORDER BY Booking_Date ,Room_ID ,Booking_STime";
...
while($row = mysqli_fetch_array($result)){
	...
	echo "<td><a href='club_booking_change.php?BID=".$row['Booking_ID']."'><img src='pencil.png' width='32' height='32' alt='modifiy' /></a></td>";
	echo "<td><a href='club_booking_delete.php?BID=".$row['Booking_ID']."'><img src='delete.png' width='32' height='32' alt='delete' /></a></td>";
	echo "</tr>";
};
echo "</table>";
mysqli_close($con);
